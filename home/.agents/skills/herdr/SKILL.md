...
Default to a new tab in the current workspace and the current working directory. Label it for the work it does. Do not create a workspace or worktree, and do not change cwd, unless the user explicitly requests that topology or location.

```bash
herdr tab create --workspace "$HERDR_WORKSPACE_ID" --cwd "$PWD" --label reviewer --no-focus
```

Read the pane to work in from `.result.root_pane.pane_id`, and the tab from `.result.tab.tab_id`.

A background tab is never seen in the focused UI, so its agent settles to `done` rather than `idle` — an unambiguous signal that the work you dispatched finished. A sibling pane in the caller's own tab inherits that tab's seen state and settles to `idle`, which is indistinguishable from an agent that was already waiting. Prefer the tab for that reason alone.

Split a pane instead only when the user wants the child visible beside the caller. Then keep focus in the calling pane and preserve cwd:

```bash
herdr pane split --current --direction right --cwd "$PWD" --no-focus
```

Split a wide pane to the right and a narrow or tall pane down; check with `herdr pane layout --pane "$HERDR_PANE_ID"`. Avoid repeated same-direction splits that create unusably narrow columns or short rows. Read the new pane ID from `.result.pane.pane_id`.
...
Dev servers, test watchers, and other long-running processes go in their own labelled tab in the current workspace, for the same reason agents do: a well-named tab is easier to spot and return to than a split pane, and it scales when two or more processes need to stay open.

```bash
herdr tab create --workspace "$HERDR_WORKSPACE_ID" --cwd "$PWD" --label dev-server --no-focus
```

Read the pane from `.result.root_pane.pane_id`, then run and inspect the command:
